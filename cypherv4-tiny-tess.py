...
def ocr(img):
    ...
    # Perform thresholding to remove noise and enhance text
    _, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    
    # Perform dilation to further enhance text
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    dilate = cv2.dilate(thresh, kernel, iterations=1)
    
    # Extract text using Tesseract
    text = pytesseract.image_to_string(dilate, config='--psm 11')
    
    # Post-processing to clean up text
    text = text.strip().replace('\n', '').replace('\r', '').replace(' ', '').upper()[:7]
    
    # Display text on image
    cv2.putText(img, text, (50, 70), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 0), 2)
    print(text)
